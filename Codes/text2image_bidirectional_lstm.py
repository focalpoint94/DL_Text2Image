...
#################################################
    ...
    def _build_model(self):
        ...
        with tf.variable_scope("rnnencoder", reuse=self.reuse):
            ...
            # RNN encoder
            LSTMCell_1 = tf.contrib.rnn.BasicLSTMCell(self.t_dim, reuse=self.reuse)
            LSTMCell_2 = tf.contrib.rnn.BasicLSTMCell(self.t_dim, reuse=self.reuse)
            initial_state_1 = LSTMCell_1.zero_state(self.batch_size, dtype=tf.float32)
            initial_state_2 = LSTMCell_2.zero_state(self.batch_size, dtype=tf.float32)
            rnn_net = tf.nn.bidirectional_dynamic_rnn(cell_fw=LSTMCell_1,
                                    cell_bw=LSTMCell_2,
                                    inputs=embedded_word_ids,
                                    initial_state_fw=initial_state_1,
                                    initial_state_bw=initial_state_2,
                                    dtype=np.float32,
                                    time_major=False,
                                    scope='rnn/dynamic')
            self.rnn_net = rnn_net
            self.outputs = rnn_net[0][0][:, -1, :] + rnn_net[0][1][:, 0, :]
