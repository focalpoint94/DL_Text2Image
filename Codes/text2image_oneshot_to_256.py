...
def max_pooling(x, pool_size=3, strides=2, padding='VALID') :
    return tf.layers.max_pooling2d(x, pool_size=pool_size, strides=strides, padding=padding)

def avg_pooling(x, pool_size=4, strides=2, padding='SAME'):
    return tf.layers.average_pooling2d(x, pool_size=pool_size, strides=strides, padding=padding)
...
#################################################
    ...
    def _build_model(self):
        ...
        with tf.variable_scope("generator", reuse=self.reuse):
            ...
            ch = 1024
            x = fully_connected(net_in, units=4*4*ch, sn=self.sn, scope='fc')
...
#################################################
    ...
    def _build_model(self):
        ...
        with tf.variable_scope("discriminator", reuse=self.reuse):
            ...
            net_h2 = Conv2d(net_h1, 4, 4, df_dim*4, 2, 2, name='d_h2/conv2d1', padding='SAME')
            net_h2 = batch_normalization(net_h2, activation_fn=tf.nn.leaky_relu, is_training=self.is_training, name='d_h2/batchnorm1')
            
            net_h2 = Conv2d(net_h2, 4, 4, df_dim*8, 2, 2, name='d_h2/conv2d2', padding='SAME')
            net_h2 = batch_normalization(net_h2, activation_fn=tf.nn.leaky_relu, is_training=self.is_training, name='d_h2/batchnorm2')
            
            net_h2 = Conv2d(net_h2, 4, 4, df_dim*8, 2, 2, name='d_h2/conv2d3', padding='SAME')
            net_h2 = batch_normalization(net_h2, activation_fn=tf.nn.leaky_relu, is_training=self.is_training, name='d_h2/batchnorm3')
            ...
            net_ho = Conv2d(net_h4, s64, s64, 1, s64, s64, name='d_ho/conv2d', biased=True) # biased = True
...
#################################################
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('cnnencoder', reuse=self.reuse):
            ...
            net_h3 = Conv2d(net_h2, 4, 4, df_dim*8, 2, 2, name='cnnf/h3/conv2d1', padding='SAME')
            net_h3 = batch_normalization(net_h3, activation_fn=tf.nn.leaky_relu, is_training=self.is_training, name='cnnf/h3/batch_norm1')
            
            net_h3 = Conv2d(net_h3, 4, 4, df_dim*8, 2, 2, name='cnnf/h3/conv2d2', padding='SAME')
            net_h3 = batch_normalization(net_h3, activation_fn=tf.nn.leaky_relu, is_training=self.is_training, name='cnnf/h3/batch_norm2')
            
            net_h3 = Conv2d(net_h3, 4, 4, df_dim*8, 2, 2, name='cnnf/h3/conv2d3', padding='SAME')
            net_h3 = batch_normalization(net_h3, activation_fn=tf.nn.leaky_relu, is_training=self.is_training, name='cnnf/h3/batch_norm3')
...
#################################################
    ...
    def _build_model(self):
        ...
        with tf.variable_scope("discriminator", reuse=self.reuse):
            ch = 64
            x = init_down_resblock(self.input_image, channels=ch, sn=self.sn, scope='init_resblock')
            ...
            ch = ch * 2
            x = down_resblock(x, channels=ch*2, sn=self.sn, scope='middle_down_resblock_1')
            ch = ch * 2
            x = down_resblock(x, channels=ch*2, sn=self.sn, scope='middle_down_resblock_2')
            x = down_resblock(x, channels=ch*2, sn=self.sn, scope='middle_down_resblock_3')
            x = down_resblock(x, channels=ch*2, sn=self.sn, scope='middle_down_resblock_4')
            ...
            x = concat([x, net_txt], axis=3, name='x_concat')
            x = down_resblock(x, channels=ch, sn=self.sn, to_down=False, scope='middle_down_resblock_3')
            x = lrelu(x, 0.2)
            x = global_sum_pooling(x)
            self.outputs = fully_connected(x, units=1, sn=self.sn, scope='d_logit')
            self.logits = x
...
#################################################
    ...
    def _build_model(self):
        with tf.variable_scope('cnnencoder', reuse=self.reuse):
            ...
            x = flatten(x, name='cnnf/flatten')
            x = fc(x, num_out=self.t_dim, name='cnnf/embed', biased=False)
            self.outputs = x
